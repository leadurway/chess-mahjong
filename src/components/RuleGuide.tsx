...
export const RuleGuide: React.FC<RuleGuideProps> = ({ onClose }) => {
  // iPad/desktop get bigger demo tiles alongside the larger text/padding applied via md:
  // breakpoints throughout this component.
  ...
  return (
    <div className="bg-stone-900/95 text-stone-100 p-6 md:p-10 rounded-2xl border border-amber-500/20 max-w-4xl md:max-w-6xl w-full mx-auto shadow-2xl overflow-y-auto max-h-[85dvh] scrollbar-thin scrollbar-thumb-stone-700">
      <div className="flex justify-between items-center mb-6 border-b border-amber-500/20 pb-4">
        <h2 className="text-2xl md:text-4xl font-bold font-serif text-amber-400 flex items-center gap-2">
          🀄 象棋麻將 遊戲規則說明
        </h2>
        {onClose && (
          <button
            onClick={onClose}
            className="text-stone-400 hover:text-white bg-stone-800 hover:bg-stone-700 font-bold px-3 py-1 md:px-5 md:py-2 md:text-xl rounded-lg transition"
          >
            關閉
          </button>
        )}
      ...
      <div className="space-y-8">
        ...
        <div>
          <h3 className="text-lg md:text-2xl font-semibold text-amber-300 mb-2 font-serif">1. 象棋麻將簡介與模式細則</h3>
          <p className="text-stone-300 text-sm md:text-lg leading-relaxed space-y-1">
            象棋麻將結合了傳統<b>中國象棋棋子</b>與<b>麻將摸打玩法的快速博弈遊戲</b>。依據子數模式不同，玩法與胡牌組合均有所調整：
          </p>
          <div className="bg-stone-800/60 p-4 md:p-6 rounded-xl border border-amber-500/10 text-xs md:text-base mt-3 space-y-2">
        ...
        <div>
          <h3 className="text-lg md:text-2xl font-semibold text-amber-300 mb-3 font-serif">2. 合法面子（搭子）組合</h3>
          <p className="text-stone-300 text-sm md:text-lg mb-4">
            「面子（搭子）」係指由三張牌構成的組合，分為「同色順子」與「同色刻子（三張相同）」：
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs md:text-base">
            {/* Red Sequences */}
            <div className="bg-stone-800/80 p-4 md:p-6 rounded-xl border border-red-500/10">
              <span className="text-red-400 font-bold text-sm md:text-lg block mb-3">🔴 紅方專屬順子</span>
            ...
            {/* Black Sequences */}
            <div className="bg-stone-800/80 p-4 md:p-6 rounded-xl border border-emerald-500/10">
              <span className="text-emerald-400 font-bold text-sm md:text-lg block mb-3">🟢 黑方專屬順子</span>
              <div className="space-y-4">
                <div>
            ...
            {/* Triples */}
            <div className="bg-stone-800/80 p-4 md:p-6 rounded-xl border border-stone-700 md:col-span-2">
              <span className="text-amber-400 font-bold text-sm md:text-lg block mb-2">⭐ 刻子（同子同色三張，如兵、卒）</span>
              <p className="text-stone-400 mb-3 text-xs md:text-base">
        ...
        <div>
          <h3 className="text-lg md:text-2xl font-semibold text-amber-300 mb-2 font-serif">3. 上碰槓機制</h3>
          <ul className="list-disc pl-5 text-stone-300 text-sm md:text-lg space-y-2">
            ...
            <table className="min-w-full text-xs md:text-base text-left text-stone-300">
              <thead className="bg-stone-750 text-amber-400 border-b border-stone-700 font-serif">
                <tr>
                  ...
                  <th className="px-4 py-3 md:px-6 md:py-4">台數</th>
        ...
        <div className="bg-amber-950/20 p-4 md:p-6 rounded-xl border border-amber-900/40 text-xs md:text-base text-stone-300">
