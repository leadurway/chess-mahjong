    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-3 items-center mb-3 shrink-0">
          <div className={`justify-self-start inline-flex items-center justify-center ${logoSizeClass} rounded-full border-2 border-amber-400 bg-[#05382a] shadow-xl`}>
            <span className={`${logoTextClass} font-serif font-black text-[#b91c1c] drop-shadow-[0_1px_1px_rgba(255,255,255,0.15)]`}>帥</span>
            <span className={`${logoTextClass} font-serif font-black text-emerald-200 -ml-1 drop-shadow-[0_1px_1px_rgba(255,255,255,0.15)]`}>將</span>
          </div>
          <h1 className={`justify-self-center ${titleTextClass} font-extrabold text-amber-250 font-serif tracking-widest leading-relaxed`}>
            象棋麻將
          </h1>
          <img
            src={liangLogo}
            alt="諒 LIANG GAME"
            className={`justify-self-end ${logoSizeClass} rounded-full border-2 border-amber-400 shadow-xl object-cover shrink-0`}
          />
        </div>
        ...
        <div className="space-y-3 shrink-0">
          ...
          {/* Player Profile */}
          <div className={`bg-white/5 border border-white/10 rounded-2xl ${sectionPadClass}`}>
          ...
          <div className={`bg-white/5 border border-white/10 rounded-2xl ${sectionPadClass}`}>
            ...
            <div className="grid grid-cols-3 gap-1">
              <button
                type="button"
                onClick={() => setMode(32)}
                className={`${modeBtnClass} rounded-xl border transition-all duration-200 cursor-pointer ${mode === 32 ? SELECTED_BTN : UNSELECTED_BTN}`}
              >
                32 子 (經典)
                <span className={`block ${modeSubTextClass} opacity-90 font-normal`}>單一副象棋</span>
              </button>

              <button
                type="button"
                onClick={() => setMode(56)}
                className={`${modeBtnClass} rounded-xl border transition-all duration-200 cursor-pointer ${mode === 56 ? SELECTED_BTN : UNSELECTED_BTN}`}
              >
                56 子 (中等)
                <span className={`block ${modeSubTextClass} opacity-90 font-normal`}>中位混合版</span>
              </button>

              <button
                type="button"
                onClick={() => setMode(64)}
                className={`${modeBtnClass} rounded-xl border transition-all duration-200 cursor-pointer ${mode === 64 ? SELECTED_BTN : UNSELECTED_BTN}`}
              >
                64 子 (雙副)
          ...
          <div className={`bg-white/5 border border-white/10 rounded-2xl ${sectionPadClass}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`${choiceBtnClass} rounded-xl border transition-all duration-200 cursor-pointer ${difficulty === 'easy' ? SELECTED_BTN : UNSELECTED_BTN}`}
              >
                輕鬆對戰 (Easy AI)
              </button>
              <button
                ...
                className={`${choiceBtnClass} rounded-xl border transition-all duration-200 cursor-pointer ${difficulty === 'hard' ? SELECTED_BTN : UNSELECTED_BTN}`}
              >
                象棋大師 (Hard AI)
              </button>
            </div>
          ...
          <div className={`bg-white/5 border border-white/10 rounded-2xl ${sectionPadClass}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`${choiceBtnClass} rounded-xl border transition-all duration-200 cursor-pointer whitespace-nowrap ${playerIsBanker ? SELECTED_BTN : UNSELECTED_BTN}`}
              >
        ...
        <div className="mt-3 pt-3 border-t border-white/10 flex flex-col gap-2 shrink-0">
          ...
          <button
            ...
            className={`w-full bg-white/5 hover:bg-white/10 text-white/95 ${rulesBtnClass} rounded-xl transition border border-white/10 cursor-pointer`}
          >
            查看規則與台數說明 (Rules)
          </button>
        </div>
